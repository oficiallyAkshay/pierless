# Handoff for pierless, 2026-09-17
...
- Repo github.com/oficiallyAkshay/pierless, local clone `~/Desktop/GitHub Repos/ProjectAstraGitOnPersonal/pierless`, plan file `~/.claude/plans/structured-marinating-popcorn.md`. Never reintroduce the gangplank name.
...
- v0.2.0 renames env vars to `PIERLESS_*`. ProjectAstraRepo then needs a second PR: bump the pin in `.github/workflows/deploy.yml`, rename `GANGPLANK_*` in `scripts/deploy-alert.sh` and `GANGPLANK_COMMITS` in deploy.yml. Astra PR #1557 re-pinned to the rewritten v0.1.1 commit; confirm it merged.
...
- Astra consumer fixed: ProjectAstraRepo PR #1557 merged, deploy.yml pins `oficiallyAkshay/pierless@60e44e5…  # v0.1.1`. `BRAHMA/GOTCHAS_GANGPLANK.md` §1 records the rename and what the v0.2.0 bump must change.
...
8. Astra PR 2 after v0.2.0: bump the pin, rename `GANGPLANK_*` in `scripts/deploy-alert.sh`, the `GANGPLANK_COMMITS` env and the `id: gangplank` step in deploy.yml, update the runner-gate test. Astra's gates need a `Map:` line in the PR body, the `smriti:pending` label swapped (used `smriti:incomplete:single-session-build-task`), and a one-bullet landing record under `BRAHMA/recent/` in the shape `- YYYY-MM-DD ET — map-sync — <branch>: …`, summary at most 400 bytes; arm auto-merge through the GraphQL mutation with SQUASH.
...
- Confirmed live: the single `ci` gate job landed (PR #8) and ruleset 23569212 now requires only `ci`. Astra PR #1557 merged on 2026-09-16 23:13 UTC.
